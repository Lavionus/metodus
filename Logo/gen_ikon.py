# Generuje sadu ikon Metodusu z vektorizovaného loga (viz build_ikon.sh).
import pathlib, subprocess, sys

S = pathlib.Path(sys.argv[1])          # pracovní složka
OUT = pathlib.Path(sys.argv[2])        # cíl (metodus/)
D = (S / 'path.txt').read_text().strip()
W, H = (int(c) for c in (S / 'rozmer.txt').read_text().split())  # rozměr předlohy
