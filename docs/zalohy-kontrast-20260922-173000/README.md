...
| `common.css`, `theme.js` | sdílený vzhled a přepínání motivů (tmavý, světlý, sépie, stará knihovna, škola, noční škola) |
| `sepia.css`, `knihovna.css`, `skola.css`, `nocni-skola.css` | palety a textury dalších motivů (importuje je `common.css`) |
...
Většina stránek linkuje `theme.js` a jde se zbytkem webu. Motivů je šest, vybírají
se z výklopného seznamu v hlavičce rozcestníku (nativní `<select id="temaVolba">`;
na úzké obrazovce se i s popiskem „Motiv“ přesune do nabídky „Další“).
...
node tests/motivy.mjs             # šest motivů: výklopný seznam, tón, tmavé simulace, textury, kontrast, stejné písmo
