...
- `IMG-aj-02` má kompletní témata Zvířata, Jídlo a Škola: 31 akvarelových karet je zapojených v `aj_slovicka` do přehledu, kartiček, zadání i zpětné vazby; společný `slovicka.js` nadále používá emoji tam, kde stránka obrazový soubor ještě nemá.
- Všech 231 ilustrací má projektový PNG originál, optimalizovaný WebP (karty do 60 kB, scény do 200 kB), přesné zadání, alt text a záznam původu v `obsah/img/kapitola-7-2.json`; souhrnná galerie je `obsah/img/kapitola-7-2.html`.
- Ověření: `tests/obrazky72-browser.mjs`, Chromium přes CDP; 21 stránek, původní režimy, načtení obrázků včetně všech 12 zvířat, 10 potravin a 9 školních motivů, světlý i tmavý motiv a šířky 1280/390/320 px. Bez běhových JS výjimek a bez vodorovného přetékání; mobilní snímky byly vizuálně zkontrolovány. Cache zvýšena na v114. Veřejné nasazení neproběhlo.
