...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys()
      ...
      // 'nodus-' je původní název webu (přejmenováno na Metodus) – její cache
      // už nikdo neobsluhuje, tak ji při první aktivaci uklidíme.
      .then(keys => Promise.all(
        keys.filter(k => (k.startsWith(PREFIX) && k !== CACHE) || k.startsWith('nodus-'))
