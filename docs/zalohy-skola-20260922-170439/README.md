...
| `common.css`, `theme.js` | sdílený vzhled a přepínání motivů (tmavý, světlý, sépie, stará knihovna) |
| `sepia.css`, `knihovna.css` | palety a textury motivů Sépie a Stará knihovna (importuje je `common.css`) |
...
Většina stránek linkuje `theme.js` a jde se zbytkem webu. Motivy jsou čtyři,
přepínač v hlavičce rozcestníku je střídá dokola:
...
| Stará knihovna | `knihovna` | `dark` | `knihovna.css` – ořechové dřevo (`obsah/textures/knihovna-drevo.webp`, procedurálně z `knihovna-drevo.py`), starozlatý akcent, patkové nadpisy |
...
node tests/motivy.mjs             # čtyři motivy: přepínač, tón, tmavé simulace, textury, kontrast
