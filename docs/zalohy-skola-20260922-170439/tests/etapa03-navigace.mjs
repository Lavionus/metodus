...
try {
  ...
  assert.equal(tema.popis, 'Motiv: Světlý', 'popis ukazuje zvolený motiv, je: ' + tema.popis);
  assert.equal(tema.ikona, '☀️', 'ikona motivu');
  assert.equal(tema.zavreno, true, 'nabídka se po volbě zavře');
  // motivy jdou dokola: světlý → sépie → stará knihovna → tmavý
  for (let i = 0; i < 3; i++) await evaluate(`btnVice.click(); btnTema.click()`);
