...
const POMUCKY = `
  ...
  window.__holaTextura = () => [...document.body.querySelectorAll('*')].filter(el => {
    ...
    return /recyklovany|knihovna-drevo/.test(s.backgroundImage) && __barva(s.backgroundColor).a === 0 && el.offsetParent !== null;
  }).length;
`;
...
try {
  ...
  // 1) Rozcestník: přepínač projde všechny čtyři motivy a pošle je do lekce v iframu.
  await nacti('index.html#obsah/eduSort.html', 'dark');
  await cekej(800);
  const kolo = [];
  for (let i = 0; i < 4; i++) {
    await evaluate(`document.getElementById('btnTema').click()`);
    await cekej(300);
    kolo.push(await evaluate(`(() => { const f = document.querySelector('iframe'); const d = f.contentDocument.documentElement;
      return { tema: MetodusTheme.get(), tone: document.documentElement.dataset.tone, popis: document.getElementById('temaPopis').textContent,
  ...
  assert.deepEqual(kolo.map(k => k.tema), ['light', 'sepia', 'knihovna', 'dark'], 'pořadí motivů: ' + JSON.stringify(kolo));
  for (const k of kolo) {
    ...
    assert.ok(k.popis.startsWith('Motiv: '), 'popisek tlačítka: ' + k.popis);
  }
  ...
  for (const cesta of ['obsah/solar_system.html', 'obsah/gravitacni_hriste.html', 'obsah/pohyb_vesmirem.html']) {
    for (const tema of ['sepia', 'knihovna']) {
    ...
    for (const tema of ['sepia', 'knihovna']) {
      ...
      const r = await evaluate(`(async () => ({ telo: __jas(__pozadi(document.body)),
        ...
        texturaNactena: await new Promise(res => { const m = getComputedStyle(document.body).backgroundImage.match(/url\\("?(.*?)"?\\)/);
          if (!m) return res(false); const i = new Image(); i.onload = () => res(i.naturalWidth > 0); i.onerror = () => res(false); i.src = m[1]; }),
      ...
      if (TONY[tema] === 'light') assert.ok(r.telo > 0.6, cesta + ' má světlý papír v sépii (jas ' + r.telo + ')');
      else assert.ok(r.telo < 0.05, cesta + ' má tmavé dřevo v knihovně (jas ' + r.telo + ')');
      assert.match(r.textura, tema === 'sepia' ? /recyklovany-papir/ : /knihovna-drevo/, cesta + ' — textura motivu');
