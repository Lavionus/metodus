...
const topic=window.ANATOMY_TOPICS.find(t=>t.id===module.topic);
$('atlas-title').textContent=topic.title;
$('topic-intro').textContent=module.intro||topic.intro;
const views=window.ANATOMY_MODULES.filter(m=>m.topic===module.topic);
$('view-switch').hidden=views.length<2;$('single-view').hidden=views.length>1;$('single-view').textContent=module.viewName;
document.querySelectorAll('[data-topic]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.topic===module.topic)));
$('view-front').setAttribute('aria-pressed',String(module.view==='front'));$('view-back').setAttribute('aria-pressed',String(module.view==='back'));
$('view-caption').textContent=module.caption;
$('map-title').textContent=`${module.title} – ${module.viewName}`;
$('flow-steps').replaceChildren();
if(module.topic==='srdce-detail')window.ANATOMY_HEART_FLOW.forEach((id,i)=>{const li=document.createElement('li'),b=document.createElement('button');b.dataset.organ=id;b.setAttribute('aria-pressed','false');b.textContent=`${i+1}. ${byId.get(id).shortName||byId.get(id).name}`;b.addEventListener('click',()=>choose(id));li.append(b);$('flow-steps').append(li);});
$('heart-flow').hidden=module.topic!=='srdce-detail'||mode==='quiz';
...
organs.forEach((o,i) => {
 ...
 (o.mirroredPaths||[]).forEach(d=>svg('path',{d,class:'hit',transform:`translate(${o.mirrorX||1024} 0) scale(-1 1)`},group));
 ...
 const [x,y]=o.label, [ax,ay]=o.anchor, labelText=o.shortName||o.name, width=o.labelWidth||(labelText.length>11?235:200);
 ...
 svg('text',{x:x+12,y:y+31},label).textContent=labelText;
...
function detail(id){const o=byId.get(id);$('detail').innerHTML=`<p class="eyebrow">${window.ANATOMY_SYSTEMS[o.system]}</p><h3>${o.name}</h3><p class="small"><i>${o.latin}</i></p><p class="lead">${o.lead}</p><dl><dt>Kde se nachází</dt><dd>${o.location}</dd><dt>Co dělá</dt><dd>${o.function}</dd><dt>S čím spolupracuje</dt><dd>${o.connection}</dd></dl><div class="remember"><strong>Zapamatuj si</strong><br>${o.remember}</div>`;}
function choose(id){
...
function setMode(next){mode=next;$('heart-flow').hidden=module.topic!=='srdce-detail'||mode==='quiz';clearStates();$('explore-mode').setAttribute('aria-pressed',String(mode==='explore'));$('quiz-mode').setAttribute('aria-pressed',String(mode==='quiz'));$('quiz-panel').hidden=mode!=='quiz';$('detail').hidden=mode==='quiz';$('system').disabled=mode==='quiz';$('labels').disabled=mode==='quiz';labels();if(mode==='quiz')start();else{quiz=null;enableChoices(true);select(selected||organs[0].id);detail(selected||organs[0].id);filter();}}
...
function changeView(index){
 if(!window.ANATOMY_MODULES[index]||module===window.ANATOMY_MODULES[index])return;
...
for(const view of ['front','back'])$('view-'+view).addEventListener('click',()=>changeView(window.ANATOMY_MODULES.findIndex(m=>m.topic===module.topic&&m.view===view)));
window.ANATOMY_TOPICS.forEach(topic=>{const button=document.createElement('button');button.dataset.topic=topic.id;button.textContent=topic.title;button.setAttribute('aria-pressed','false');button.addEventListener('click',()=>changeView(window.ANATOMY_MODULES.findIndex(m=>m.topic===topic.id)));$('topic-switch').append(button);});
render();choose('srdce');
})();
