...
(() => {
  ...
  function speech() {
    root.innerHTML = `<h2>Jeden rozhovor, dva vypravěči</h2><p><b>Pondělí, knihovna:</b> paní učitelka říká žákovi:</p>
      <blockquote lang="en">“I will meet you here tomorrow.”</blockquote>
      <p><b>Středa, škola:</b> rozhovor převyprávíme spolužákovi. V této ukázce použijeme posun času po <span lang="en">told</span>.</p>
      <label for="narrator">Kdo teď vypráví?</label><select id="narrator"><option value="adresat">žák (původní adresát)</option><option value="mluvci">učitelka (původní mluvčí)</option></select>
    ...
    let step = 0;
    const render = () => {
      const adresat = $('narrator').value === 'adresat';
      const tokens = [adresat ? 'she' : 'I', 'would', adresat ? 'me' : 'him', 'there', 'the next day'];
      const original = ['I','will','you','here','tomorrow'];
      const part = i => step > i ? `<mark>${tokens[i]}</mark>` : original[i];
      $('speechSentence').innerHTML = step === 5 ? `${adresat ? 'The teacher told me' : 'I told the student'} (that) ${tokens[0]} ${tokens[1]} meet ${tokens[2]} ${tokens[3]} ${tokens[4]}.` : `${part(0)} ${part(1)} meet ${part(2)} ${part(3)} ${part(4)}.`;
      const explanations = ['Nejdřív urči, kdo je I: v původní větě učitelka.', adresat ? 'I → she: žák mluví o učitelce.' : 'I zůstává I: učitelka mluví sama o sobě.', 'will → would: v této úloze posouváme čas.', adresat ? 'you → me: žák byl adresát a teď mluví o sobě.' : 'you → him: učitelka mluví o žákovi.', 'here → there: knihovna je jinde než místo vyprávění.', 'tomorrow → the next day: stále úterý. Celá věta používá told + adresát bez to.'];
    ...
    $('speechCheck').onclick = () => {
      ...
      feedback(!day ? 'Nejdřív vyber den.' : day === 'tue' ? 'Správně: učitelka v pondělí myslela úterý. Vyprávění ve středu tento den nemění.' : 'Čtvrtek by byl zítřek vypravěče ve středu. Původní tomorrow ale označovalo úterý.');
    };
    $('pilotReset').onclick = () => {step=0; $('narrator').value='adresat'; root.querySelectorAll('[name=day]').forEach(x=>x.checked=false); render(); feedback('Projdi změny a urči den schůzky.');};
  ...
  document.querySelectorAll('a[data-lekce]').forEach(a=>a.addEventListener('click',e=>{
    ...
    window.parent.postMessage({otevri:'obsah/'+a.getAttribute('href')},cilovyPuvod);
  }));
})();
