...
try {
  ...
  for(const file of ['ch9_ph','aj9_neprima_rec','d6_prameny','inf6_tabulky']) {
    ...
    if(file==='ch9_ph') {
      ...
      assert.equal(await evaluate(`document.getElementById('speechSentence').textContent`),'The teacher told me (that) she would meet me there the next day.');
      await evaluate(`document.getElementById('narrator').value='mluvci';document.getElementById('narrator').dispatchEvent(new Event('change'))`);
      for(let i=0;i<5;i++)await click('speechNext');
      assert.equal(await evaluate(`document.getElementById('speechSentence').textContent`),'I told the student (that) I would meet him there the next day.');
